...
struct HowToPlayView: View {
    private let sections: [(icon: AnyView, title: String, body: String)] = [
        (AnyView(BookIcon(color: Scribe.brassDeep)), "The Goal",
        ...
        (AnyView(ScribeIcon(color: Scribe.brassDeep)), "Entering Letters",
                        ...
                        ScribeCard {
                            HStack(alignment: .top, spacing: 14) {
                                ...
                                VStack(alignment: .leading, spacing: 5) {
                                    Text(sections[i].title).font(Scribe.titleBold(17)).foregroundColor(Scribe.ink)
                                    Text(sections[i].body).font(Scribe.body(14)).foregroundColor(Scribe.inkSoft)
...
private struct ChevronIconWrap: View {
    var body: some View {
        HStack(spacing: 2) {
            ChevronIcon(color: Scribe.brassDeep).frame(width: 14, height: 14).rotationEffect(.degrees(180))
            ChevronIcon(color: Scribe.brassDeep).frame(width: 14, height: 14)
        }
    }
}
